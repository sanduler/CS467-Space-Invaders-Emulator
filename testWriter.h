...
int OPCODES_TO_WRITE_START  = 0;        // Defines which opcode call to start writing on (can bve changed)
int OPCODES_TO_WRITE_END    = 10000;    // Defines which opcode call to stop writing on  (can be changed)
...
void writeOpcode (uint8_t opcode, uint16_t pc, uint8_t cycles, uint16_t sp, 
    ...
    if (times_ran == 0) {
        TEST_OUT_FILE.open("Opcode_Log.txt", ios::out | ios::trunc);
