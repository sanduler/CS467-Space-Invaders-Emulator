...
void test_opCode(unsigned char passed_code) {
    switch(passed_code) {
        ...
        case 0x3e:
            // A <- byte 2
            cs->reg_A.set(cs->opCode_Array[1]);
            break;
        case 0x3f:
            // CY=!CY
            incomplete();
        ...
        case 0x40:
            // B <- B
            cs->reg_B.set(cs->reg_B.get());
            break;
        case 0x41:
            // B <- C
            cs->reg_B.set(cs->reg_C.get());
            break;
        case 0x42:
            // B <- D
            cs->reg_B.set(cs->reg_D.get());
            break;
        case 0x43:
            // B <- E
            cs->reg_B.set(cs->reg_E.get());
            break;
        case 0x44:
            // B <- H
            cs->reg_B.set(cs->reg_H.get());
            break;
        case 0x45:
            // B <- L
            cs->reg_B.set(cs->reg_L.get());
            break;
        case 0x46:
            // B <- (HL)
            incomplete();
        ...
        case 0x47:
            // B <- A
            cs->reg_B.set(cs->reg_A.get());
            break;
        case 0x48:
            // C <- B
            cs->reg_C.set(cs->reg_B.get());
            break;
        case 0x49:
            // C <- C
            cs->reg_C.set(cs->reg_C.get());
            break;
        case 0x4a:
            // C <- D
            cs->reg_C.set(cs->reg_D.get());
            break;
        case 0x4b:
            // C <- E
            cs->reg_C.set(cs->reg_E.get());
            break;
        case 0x4c:
            // C <- H
            cs->reg_C.set(cs->reg_H.get());
            break;
        case 0x4d:
            // C <- L
            cs->reg_C.set(cs->reg_L.get());
            break;
        case 0x4e:
            // C <- (HL)
            incomplete();
        ...
        case 0x4f:
            // C <- A
            cs->reg_C.set(cs->reg_A.get());
            break;
        case 0x50:
            // D <- B
            cs->reg_D.set(cs->reg_B.get());
            break;
        case 0x51:
            // D <- C
            cs->reg_D.set(cs->reg_C.get());
            break;
        case 0x52:
            // D <- D
            cs->reg_D.set(cs->reg_D.get());
            break;
        case 0x53:
            // D <- E
            cs->reg_D.set(cs->reg_E.get());
            break;
        case 0x54:
            // D <- H
            cs->reg_D.set(cs->reg_H.get());
            break;
        case 0x55:
            // D <- L
            cs->reg_D.set(cs->reg_L.get());
            break;
        case 0x56:
            // D <- (HL)
            incomplete();
        ...
        case 0x57:
            // D <- A
            cs->reg_D.set(cs->reg_A.get());
            break;
        case 0x58:
            // E <- B
            cs->reg_E.set(cs->reg_B.get());
            break;
        case 0x59:
            // E <- C
            cs->reg_E.set(cs->reg_C.get());
            break;
        case 0x5a:
            // E <- D
            cs->reg_E.set(cs->reg_D.get());
            break;
        case 0x5b:
            // E <- E
            cs->reg_E.set(cs->reg_E.get());
            break;
        case 0x5c:
            // E <- H
            cs->reg_E.set(cs->reg_H.get());
            break;
        case 0x5d:
            // E <- L
            cs->reg_E.set(cs->reg_L.get());
            break;
        case 0x5e:
            // E <- (HL)
            incomplete();
        ...
        case 0x5f:
            // E <- A
            cs->reg_E.set(cs->reg_A.get());
            break;
        case 0x60:
            // H <- B
            cs->reg_H.set(cs->reg_B.get());
            break;
        case 0x61:
            // H <- C
            cs->reg_H.set(cs->reg_C.get());
            break;
        case 0x62:
            // H <- D
            cs->reg_H.set(cs->reg_D.get());
            break;
        case 0x63:
            // H <- E
            cs->reg_H.set(cs->reg_E.get());
            break;
        case 0x64:
            // H <- H
            cs->reg_H.set(cs->reg_H.get());
            break;
        case 0x65:
            // H <- L
            cs->reg_H.set(cs->reg_L.get());
            break;
        case 0x66:
            // H <- (HL)
            incomplete();
        ...
        case 0x67:
            // H <- A
            cs->reg_H.set(cs->reg_A.get());
            break;
        case 0x68:
            // L <- B
            cs->reg_L.set(cs->reg_B.get());
            break;
        case 0x69:
            // L <- C
            cs->reg_L.set(cs->reg_C.get());
            break;
        case 0x6a:
            // L <- D
            cs->reg_L.set(cs->reg_D.get());
            break;
        case 0x6b:
            // L <- E
            cs->reg_L.set(cs->reg_E.get());
            break;
        case 0x6c:
            // L <- H
            cs->reg_L.set(cs->reg_H.get());
            break;
        case 0x6d:
            // L <- L
            cs->reg_L.set(cs->reg_L.get());
            break;
        case 0x6e:
            // L <- (HL)
            incomplete();
        ...
        case 0x78:
            // A <- B
            cs->reg_A.set(cs->reg_B.get());
            break;
        case 0x79:
            // A <- C
            cs->reg_A.set(cs->reg_C.get());
            break;
        case 0x7a:
            // A <- D
            cs->reg_A.set(cs->reg_D.get());
            break;
        case 0x7b:
            // A <- E
            cs->reg_A.set(cs->reg_E.get());
            break;
        case 0x7c:
            // A <- H
            cs->reg_A.set(cs->reg_H.get());
            break;
        case 0x7d:
            // A <- L
            cs->reg_A.set(cs->reg_L.get());
            break;
        case 0x7e:
            // A <- (HL)
            incomplete();
        ...
        case 0x80:
            // A <- A + B
            cs->reg_A.set(cs->reg_A.get() + cs->reg_B.get());
            break;
        case 0x81:
            // A <- A + C
            cs->reg_A.set(cs->reg_A.get() + cs->reg_C.get());
            break;
        case 0x82:
            // A <- A + D
            cs->reg_A.set(cs->reg_A.get() + cs->reg_D.get());
            break;
        case 0x83:
            // A <- A + E
            cs->reg_A.set(cs->reg_A.get() + cs->reg_E.get());
            break;
        case 0x84:
            // A <- A + H
            cs->reg_A.set(cs->reg_A.get() + cs->reg_H.get());
            break;
        case 0x85:
            // A <- A + L
            cs->reg_A.set(cs->reg_A.get() + cs->reg_L.get());
            break;
        case 0x86:
            // A <- A + (HL)
            incomplete();
        ...
        case 0x87:
            // A <- A + A
            cs->reg_A.set(cs->reg_A.get() + cs->reg_A.get());
            break;
        case 0x88:
            // A <- A + B + CY
            cs->reg_A.set(cs->reg_A.get() + cs->reg_B.get());
        ...
        case 0xc1:
            // C <- (sp); B <- (sp+1); sp <- sp+2
            cs->reg_C.set(cs->reg_SP.get());
            cs->reg_B.set(cs->reg_SP.get() + 1);
            cs->reg_SP.set(cs->reg_SP.get() + 2);
            break;
        case 0xc2:
            // if NZ, PC <- adr
            incomplete();
